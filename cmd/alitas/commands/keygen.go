...
import (
	...
	"github.com/AlitasTech/Alitas/src/crypto/keys"
...
var (
	privKeyFile           string
	pubKeyFile            string
	defaultPrivateKeyFile = fmt.Sprintf("%s/priv_key", _config.Alitas.DataDir)
	defaultPublicKeyFile  = fmt.Sprintf("%s/key.pub", _config.Alitas.DataDir)
)
...
func keygen(cmd *cobra.Command, args []string) error {
	if _, err := os.Stat(privKeyFile); err == nil {
		return stacktrace.NewError("A key already lives under: %s", path.Dir(privKeyFile))
	}

	key, err := keys.GenerateECDSAKey()
	if err != nil {
		return stacktrace.NewError("Error generating ECDSA key")
	}

	if err := os.MkdirAll(path.Dir(privKeyFile), 0700); err != nil {
		return stacktrace.NewError("Writing private key: %s", err)
	}
	...
	if err := os.MkdirAll(path.Dir(pubKeyFile), 0700); err != nil {
		return stacktrace.NewError("Writing public key: %s", err)
	}
