// Package config defines the configuration for a Alt node.
//
// Regardless of how Alt is started, directly from Go code or as a standalone
...
// configuration options, Alt relies on a data directory, defined by
// Config.DataDir, where it expects to find a few additional configuration
// files:
//
//  priv_key // a plain text file containing the raw private key (cf. alt keygen).
